...
var isDevelop = !(currentPath.indexOf('develop') == -1);    //是否是项目结构
var tmpPath = path.posix.normalize(currentPath.substring(0, currentPath.indexOf('develop'))).replace(/\//g, '\\');
var production = isDevelop ? (/(\w+)_(\w+)_(\w+)\\$/gmi.exec(tmpPath))[2] : '';  //业务线
var releasePath = isDevelop ? currentPath.substring(currentPath.indexOf('develop')+8) : currentPath;  //项目发布路径
...
module.exports = function (src, dest, projectName, projectVersion, done){
    ...
    var projectRelativePath = isDevelop ? production+'/'+dest.substring(dest.indexOf(releasePath)).replace(/\\/g, '/') :
        currentPath.replace(/\\/g, '/').replace(/(\w|\W)\:\//, '');

    if(releasePath){    //相对发布路径逻辑
